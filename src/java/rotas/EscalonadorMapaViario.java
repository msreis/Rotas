...
public class EscalonadorMapaViario extends Escalonador{
    ...
    public EscalonadorMapaViario(Semaforo sinal, MapaViarioRotas mapa, String freq){
	super(freq);
	this.semaforo = sinal;
	this.mapa = mapa;
    }	
	...
	while(!_pare){
		...
		Log.escreveLog("rotas.log", "Inicializando atualizacao do mapa viario (ultima atualizacao em " + this.mapa.devolveHorarioUltimaAtualizacao() + ")");	    
		this.mapa.atualizaMapaViario();
		Log.escreveLog("rotas.log", "Mapa atualizado em " + this.mapa.devolveHorarioUltimaAtualizacao());
