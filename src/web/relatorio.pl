...
# relatorio.pl -- CGI para gerar relatorio de clientes no sistema Rotas.
#
# M.S.Reis, 21.03.09
...
for(my $i = 0; $i<$count; $i++){
	...
	printf "<li><em>Nome:</em> %s<br><em>CPF:</em> %s<br><em>Rotas detalhadas:</em><br><ul>", $nome[$i], $cpf[$i]; 
	...
	for(my $j=0; $j<$num_rotas; $j++){
		...
		printf "<li>Rota # %d. Vias utilizadas:<br>", $rota[$j]; 
		...
		$sql = qq{  SELECT nomeVia, Via.noInicial, Via.noFinal, horaConsultaVia 
        	...
        	while( $sth->fetch() ) {
			printf "'%s %d-%d', em %s<br>", $nomeVia, $noInicial, $noFinal, $horaConsultaVia;
        	}
